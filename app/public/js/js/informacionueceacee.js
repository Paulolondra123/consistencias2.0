...
const obtenerToken = async () => {
  ...
  // Function to fetch data from /proxy and load it into the iframe
  const cargarDatosEnIframe = async () => {
    const token = obtenerTokenre();
    if (!token) return;
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.error('Error al hacer la solicitud proxy:', response.statusText);
        }
    } catch (error) {
...
// Load data into iframe when the page loads
window.onload = cargarDatosEnIframe;
